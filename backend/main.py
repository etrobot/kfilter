...
def load_startup_config():
    ...
    try:
        ...
        if config_data:
            ...
            if (
                all(
                    val and val not in ["your_bearer_token_here", "your_user_id_here"]
                    for val in zai_values[:2]
                )
                and zai_values[2]
            ):
            ...
            config_status = (
                f"Loaded configuration from config.json: {len(config_data)} settings"
            )
            if valid_configs:
            ...
            logging.info(
                "No config.json found or empty, using environment variables only"
            )
            return False
    except Exception as e:
...
from api import (
    ...
    list_all_tasks,
    collect_concepts,
    get_concept_task_status,
    get_latest_concept_results,
    list_all_concept_tasks,
    get_concepts_list,
    stop_analysis,
    get_kline_amplitude_dashboard,
    run_extended_analysis,
    stop_extended_analysis,
    login_user,
    get_zai_config,
    update_zai_config,
    get_extended_analysis_task_status,
...
def check_system_configuration():
    ...
    try:
        ...
        logger.info(
            f"  - User ID: {'✓ Set' if user_id and user_id != 'your_user_id_here' else '✗ Not configured'}"
        )
        logger.info(
            f"  - ZAI Status: {'✓ CONFIGURED' if zai_config else '✗ NOT CONFIGURED'}"
        )
        ...
        logger.info(
            f"  - Base URL: {base_url if base_url else 'Default (https://api.openai.com/v1)'}"
        )
        logger.info(f"  - Model: {model}")
        logger.info(
            f"  - OpenAI Status: {'✓ CONFIGURED' if openai_config else '✗ NOT CONFIGURED'}"
        )

        # Overall system status
        logger.info(
            f"Overall System Status: {'✓ READY FOR OPERATION' if system_ready else '✗ REQUIRES CONFIGURATION'}"
        )

        if not system_ready:
            logger.warning("⚠ SYSTEM NOT FULLY CONFIGURED")
            logger.warning(
                "  Please configure missing components via the /config/zai endpoint"
            )
            if not zai_config:
                logger.warning(
                    "  - ZAI credentials are required for market data access"
                )
