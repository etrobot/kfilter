...
def run_analysis(request: RunRequest) -> RunResponse:
    ...
    task_id = create_analysis_task(
        request.top_n, request.selected_factors, request.collect_latest_data
    )
...
def collect_concepts(clear_db: bool = False) -> RunResponse:
    ...
    """
    task_id = create_concept_collection_task(clear_db=clear_db)
    ...
    return RunResponse(task_id=task_id, status=TaskStatus.PENDING, message=message)
...
def get_latest_concept_results() -> ConceptTaskResult | Message:
    ...
    if not last_task:
        return Message(
            message="No concept collection results yet. POST /concepts/collect to start collection."
        )
...
def get_extended_analysis_results():
    ...
    if os.path.exists(json_file):
        try:
            ...
            logger.warning(
                f"Failed to load extended analysis results from {json_file}: {e}"
            )
...
def run_extended_analysis_stream():
    ...
    from data_management.services import cache_extended_analysis
    from extended_analysis import run_standalone_extended_analysis
    from data_management.chart_data_generator import (
        generate_category_based_sunburst_chart_data,
    )
    ...
    def worker():
        try:
            ...
            res = run_standalone_extended_analysis(
                on_progress=_on_progress, stop_event=stop_event
            )
            ...
            if stop_event.is_set():
                ...
                if res and "sectors" in res and res["sectors"]:
                    try:
                        sunburst_data = generate_category_based_sunburst_chart_data(
                            res["sectors"]
                        )
                        res["sunburst_data"] = sunburst_data
                        result_holder["last_msg"] = "生成旭日图数据完成"
                        update_extended_analysis_task(
                            task_id, message="生成旭日图数据完成"
                        )
                    except Exception as e:
    ...
    def event_stream():
        ...
        if result_holder["error"] is not None:
            yield format_event(
                "error",
                {"ok": False, "error": result_holder["error"], "task_id": task_id},
            )
        else:
            ...
            try:
                ...
                pass
            yield format_event(
                "complete", {"ok": True, "result": result, "task_id": task_id}
            )
...
def stop_extended_analysis(task_id: str) -> dict:
    ...
    if not stop_event:
        raise HTTPException(
            status_code=404,
            detail="Extended analysis task not found or already finished",
        )
    ...
    # Update task status
    update_extended_analysis_task(
        task_id, status="stopping", message="已请求停止扩展分析，正在清理..."
    )
...
def update_zai_config(config_data: dict) -> dict:
    ...
    missing_fields = [
        field for field in required_fields if not new_config.get(field, "").strip()
    ]
    ...
    try:
        ...
        with get_session() as session:
            ...
            if user:
                ...
                return AuthResponse(success=False, message="用户不存在，请联系管理员")
